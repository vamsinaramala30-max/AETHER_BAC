...
export class AnalyticsController {
  ...
  public exportAnalytics = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const dateRange = this.analyticsService.resolveDateRange(query.preset, query.startDate, query.endDate);

      const result = await this.analyticsService.exportAnalytics(userId, userEmail, dateRange, query.format);

      if (query.format === 'csv') {
        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', `attachment; filename="analytics-${query.preset ?? '7d'}.csv"`);
